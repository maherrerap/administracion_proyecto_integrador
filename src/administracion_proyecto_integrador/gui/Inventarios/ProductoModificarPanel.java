...
public class ProductoModificarPanel extends JPanel {
    ...
    public ProductoModificarPanel(ProductosGUI padre, String idProducto) {
    ...
    private JComponent crearFormulario() {
        ...
        gbc.gridy = row;
        gbc.weightx = 0.5;
        campos.add(crearCampoTexto("Saldo Inicial:", txtSaldoInicial = new JTextField(), false), gbc);

        gbc.gridx = 1;
        campos.add(crearCampoTexto("Ingresos:", txtIngresos = new JTextField(), false), gbc);
        ...
        gbc.gridy = row;
        gbc.weightx = 0.5;
        campos.add(crearCampoTexto("Egresos:", txtEgresos = new JTextField(), false), gbc);

        gbc.gridx = 1;
        campos.add(crearCampoTexto("Saldo Final:", txtSaldoFinal = new JTextField(), false), gbc);
    ...
    private JComponent crearPanelBotones() {
        ...
        btnGuardar.addActionListener(e -> onGuardarCambios());
    ...
    private void cargarDatosProducto(String idProducto) {
        try {
            // Obtener el producto desde la base de datos pasando por dp.
            productoActual = Productos.obtenerProductoPorId(idProducto);
            ...
            txtIngresos.setText(String.valueOf(productoActual.getProQtyIngresos()));
            txtEgresos.setText(String.valueOf(productoActual.getProQtyEgresos()));
            txtSaldoFinal.setText(String.valueOf(productoActual.getProSaldoFinal()));

            // Seleccionar la unidad de medida de compra en el combobox
            seleccionarUnidadMedida(cmbUmCompra, productoActual.getProUmCompra());
            
            // Seleccionar la unidad de medida de venta en el combobox
            seleccionarUnidadMedida(cmbUmVenta, productoActual.getProUmVenta());
            
            // Seleccionar la categoría en el combobox
            seleccionarCategoria(cmbCategoria, productoActual.getIdcategoria());
    ...
    private void seleccionarUnidadMedida(JComboBox<Productos.UnidadMedida> combo, String idUnidad) {
        for (int i = 0; i < combo.getItemCount(); i++) {
            Productos.UnidadMedida um = combo.getItemAt(i);
            if (um != null && um.getId().equals(idUnidad)) {
    ...
    private void seleccionarCategoria(JComboBox<Productos.Categoria> combo, String idCategoria) {
        for (int i = 0; i < combo.getItemCount(); i++) {
            Productos.Categoria cat = combo.getItemAt(i);
            if (cat != null && cat.getId().equals(idCategoria)) {
                combo.setSelectedIndex(i);
                break;
            }
        }
    ...
    private void onGuardarCambios() {
        try {
            ...
            productoActual.setProDescripcion(txtDescripcion.getText().trim());
            ...
            productoActual.setProUmCompra(umCompra.getId());
            productoActual.setProUmVenta(umVenta.getId());           
            productoActual.setProValorCompra(Double.parseDouble(txtValorCompra.getText().trim()));
            productoActual.setProPrecioVenta(Double.parseDouble(txtPrecioVenta.getText().trim()));
            Productos.Categoria categoria = (Productos.Categoria) cmbCategoria.getSelectedItem();
            productoActual.setIdcategoria(categoria.getId());
            
            // Validar producto usando el método verificarProd()
            List<Productos.ErrorValidacion> errores = productoActual.verificarProd();
